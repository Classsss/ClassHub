    ...
    public class PracticeListController : ControllerBase {
        ...
        public List<Submission> GetPracticeList(int room_id,int student_id) {
            ...
            foreach (CodeAssignment codeAssignment in codeAssignments) {
                ...
                practiceList.Add(new Assignment {Id = codeAssignment.assignment_id, Title = codeProblem.title, Description = codeProblem.contents, Author = codeProblem.author, StartDate = codeAssignment.start_date, EndDate = codeAssignment.end_date, IsSubmitted = is_submit });
            }
            return practiceList;
        }
    }
